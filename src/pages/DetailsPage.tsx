...
//   const handlePlayVideo = (d:any)=>{
//     setPlayVideoId(d)
//     setPlayVideo(true)

//   }

//   const duration : Cdata = (data?.runtime/60)?.toFixed(1)?.split(".")
...
//           <div>
//               <HorizontalScollCard data={similarData} heading={"Similar "+params?.explore} media_type={params?.explore} trending={false}/>
//               <HorizontalScollCard data={recommendationData} heading={"Recommendation "+params?.explore} media_type={params?.explore} trending={false}/>
//           </div>
...
const DetailsPage = () => {
  const params = useParams<{ explore: string; id: string }>();
  const imageURL = useSelector((state: RootState) => state.movieoData.imageURL);
  const { data } = useFetchDetails<Cdata>(`/${params?.explore}/${params?.id}`);
  const { data: castData } = useFetchDetails<CastData>(`/${params?.explore}/${params?.id}/credits`);
  const { data: similarData } = useFetch(`/${params?.explore}/${params?.id}/similar`);
  const { data: recommendationData } = useFetch(`/${params?.explore}/${params?.id}/recommendations`);
  const [playVideo, setPlayVideo] = useState(false);
  const [playVideoId, setPlayVideoId] = useState<Cdata | null>(null);

  const handlePlayVideo = (d: Cdata) => {
    setPlayVideoId(d);
    setPlayVideo(true);
  };
